...
class mmsenet():
    ...
    def vl_model(self, Received_Y,  Hls, Ideal_H, Ideal_X, Transmit_X, n_iter):
        with torch.no_grad():
            ...
            ff = 0
            NumSample = 22500 
            for j in range(9): #对不同的信噪比分别vl model 一共9个信噪比 把数据分开来做
                id_valid = torch.arange(int(NumSample*0.36)+j,int(NumSample*0.6*0.8),9).long()
                ...
                ber_vl = 0.0
                bler_vl = 0.0
                batch_arange=int(NumSample*0.12/9/batchsize)
                for i in range(batch_arange):
                ...
                ber_vl = ber_vl/(batch_arange)
                bler_vl = bler_vl/(batch_arange)
...
class MmseNet(nn.Module):
    ...
    def forward(self, Received_Y, Hls, Transmit_X):
        ...
        Received_Y = Received_Y.detach() #共用内存但没有梯度
        Hls = Hls.detach()
        Received_Yc = torch.complex(Received_Y[:,:,:,:,0],Received_Y[:,:,:,:,1]) #复数tensor
        Received_Yflat = Received_Y.view(Received_Y.shape[0],S,F,8).permute(0,3,1,2).float() #batch_size*8*s*f
        Hlsflat = Hls.view(Hls.shape[0],S,F,8).permute(0,3,1,2).float() #batch_size*8*s*f
        Transmit_Xflat = Transmit_X.permute(0,3,1,2).float() #batch_size*2*s*f
        output = self.net[0](torch.cat([Received_Yflat, Transmit_Xflat, Hlsflat], dim=1)) #超网络算方差
        c = self.net[1](torch.cat([Received_Yflat, Transmit_Xflat, Hlsflat], dim=1)) # 频域相关系数
        j0 = self.net[2](torch.cat([Received_Yflat, Transmit_Xflat, Hlsflat], dim=1)) # 时域相关系数
        Rf = torch.complex(batch_toeplitz(c[:,0,:],c[:,0,:]),batch_toeplitz(c[:,1,:],-c[:,1,:])) #形成相关矩阵
        ...
        Rfeye1 = Rf1 + torch.einsum('ij,k->kij', torch.eye(int(F/2)).cuda(), output[:,0]**2) #相关+噪声

        Wf_par = torch.matmul(Rf12,torch.linalg.inv(Rfeye1)).to(torch.complex128) #求逆
        Hlsc0 = torch.complex(Hls[:,0,::2,:,0],Hls[:,0,::2,:,1]).cuda() #复数LS信道
        hc_symb0 = torch.matmul(Wf_par,Hlsc0).cuda() #维纳滤波出来的信道
        h_symb0 = torch.stack([hc_symb0.real,hc_symb0.imag],dim = 1).float().permute(0,2,3,1)[:,None,:,:,:]
        H = h_symb0.repeat(1,S,1,1,1)
        Hc = torch.complex(H[:,:,:,:,0],H[:,:,:,:,1]) #粗估计的信道
