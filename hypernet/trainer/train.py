...
class Trainer():
    ...
    def _set_optim(self):
        ...
        optimizer = optim.AdamW(para_lst, lr=self.lr)
    ...
    def tr(self):
        ...
        S = 12
        F = 24
        Received_Y = np.empty([NumSample,S,F,4,2]) #接收
        ...
        #db = -5
        #网络不需要知道导频是多少 只用接受的Y 初始的Hls和发送的X 不同的多普勒放在一起
        ...
        id = np.argwhere(id % (NumSample/1000) < int(NumSample/1000*0.4)) #这里取后60%用于训练
        Received_Y = torch.from_numpy(np.delete(Received_Y,id,0)) #表示沿着第0个维度删除id中的除750余前300的
        Hls = torch.from_numpy(np.delete(Hls,id,0))
        Ideal_H = torch.from_numpy(np.delete(Ideal_H,id,0))
        Ideal_X = torch.from_numpy(np.delete(Ideal_X,id,0))       #完美的X
        Transmit_X = torch.from_numpy(np.delete(Transmit_X,id,0)) #粗估计的X
        train_size = int(NumSample*0.36)
        ...
        with torch.autograd.set_detect_anomaly(True):
            for epoch in range(start, self.epoch):
                ...
                for i in range(train_size//self.batch_size):
                    ...
                    self.optimizer.step()
                    n_iter += 1
                    print('{}/{} '.format(i,train_size//self.batch_size))
                    '''
